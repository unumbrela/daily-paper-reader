...
一个智能的 arXiv / openreview 论文推荐系统：Fork 后每天自动抓取论文、推荐论文。每日平均成本￥0.2。
...
以下内容需在**自己仓库**下执行（因为下面链接均为相对目录），并不是在原仓库中执行

### 2. 开启 Actions
进入 Fork 的仓库，页面上方点 `Actions`，把左手边 `daily-paper-reader` 开启
