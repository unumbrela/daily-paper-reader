...
title_zh: 基于多值逻辑的深度 ReLU 神经网络的完全识别
authors: "Yani Zhang, Helmut Bölcskei"
date: 2026-01-30
pdf: "https://arxiv.org/pdf/2602.00266v1"
tags: ["query:sr"]
score: 6.0
evidence: 神经网络的符号逻辑表示
tldr: 本研究针对深度ReLU神经网络的函数对称性问题，提出了基于Lukasiewicz多值逻辑的完全识别框架。通过将网络转化为逻辑公式，利用逻辑公理进行代数重写，并结合组合范式实现逻辑到网络的逆向映射。研究证明了同一函数等价类中的所有ReLU网络均可通过有限次逻辑对称变换相互转化，类比了香农在开关电路与布尔逻辑上的开创性工作，为神经网络的代数分析奠定了理论基础。
motivation: 旨在解决深度ReLU神经网络中由于函数对称性导致的多个不同参数架构对应同一函数的问题。
method: 将ReLU网络翻译为Lukasiewicz逻辑公式，并利用逻辑公理进行代数重写及组合范式转换。
result: 基于Chang完备性定理，证明了同一等价类中的所有ReLU网络都由一组有限的逻辑对称变换所连接。
conclusion: 该研究建立了ReLU网络与多值逻辑的完备映射关系，为神经网络的结构识别与化简提供了严谨的逻辑代数框架。
---

## 摘要
深度 ReLU 神经网络具有非平凡的函数对称性：截然不同的架构和参数（权重和偏置）可以实现相同的函数。我们解决了完全识别问题——即给定一个函数 f，推导出所有产生 f 的前馈 ReLU 网络的架构和参数。我们将 ReLU 网络转换为 Lukasiewicz 逻辑公式，并通过受逻辑公理支配的代数重写来实现函数等效的网络变换。我们提出了一种组合范式，以促进从 Lukasiewicz 逻辑公式到 ReLU 网络的回映射。利用 Chang 完备性定理，我们证明了对于每个函数等价类，该类中的所有 ReLU 网络都通过一组有限的对称性相连，这些对称性对应于 Lukasiewicz 逻辑的有限公理集。这一思想让人联想到香农在开关电路设计方面的开创性工作，其中电路被转换为布尔公式，并通过受布尔逻辑公理支配的代数重写来实现综合。
