...
## 摘要
综述目的：影像衍生血流储备分数（FFR）正迅速从传统的基于计算流体力学（CFD）的流程，向机器学习（ML）、深度学习（DL）及物理信息方法演进，从而实现对冠状动脉狭窄进行快速、无导丝且可扩展的功能性评估。本综述综合了基于 CT 和血管造影的 FFR 的最新进展，特别强调了新兴的物理信息神经网络和神经算子（PINNs 和 PINOs）以及其临床转化的关键考量。最近研究发现：ML/DL 方法显著提高了自动化程度和计算速度，能够根据解剖描述符或血管造影对比剂动力学预测压力和 FFR。然而，由于多中心异质性、可解释性挑战以及采集协议和图像质量的差异，其在真实世界中的表现和泛化能力可能仍存在波动，且对领域偏移（domain shift）较为敏感。物理信息学习在模型训练中引入了守恒结构和边界条件一致性，在保持快速推理的同时，提高了泛化能力并减少了对密集监督的依赖。近期的评估趋势日益强调面向部署的指标，包括校准、不确定性量化和质量控制门控，这些对于临床安全使用至关重要。总结：该领域正趋向于开发更快速、更自动化且更可靠的影像衍生 FFR 方法。虽然 ML/DL 带来了显著的效率提升，但诸如 PINNs 和 PINOs 等物理信息框架可能在速度与物理一致性之间提供更稳健的平衡。前瞻性多中心验证和标准化评估对于支持广泛且安全的临床应用至关重要。
